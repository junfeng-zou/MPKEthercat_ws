  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Queue SDO configuration onto each slave_config. The IgH master will
  // execute these downloads while walking each slave from PREOP to SAFEOP
  // during master_.activate(), which is the only state in which the SDO
  // gateway is reliably available. Live downloads via ecrt_master_sdo_download
  // (the previous behaviour) typically fail at this point with EINVAL because
  // the bus has just been scanned and SDO addressing is not yet ready.
  for (auto i = 0ul; i < ec_modules_.size(); i++) {
    const uint16_t slave_pos =
      static_cast<uint16_t>(std::stod(ec_module_parameters_[i]["position"]));
    for (auto & sdo : ec_modules_[i]->sdo_config) {
      int ret = master_.queueSlaveSdo(slave_pos, sdo);
      if (ret) {
        RCLCPP_WARN(
          rclcpp::get_logger("EthercatDriver"),
          "Failed to queue config SDO 0x%04X:%u for module at position %u "
          "(ecrt_slave_config_sdo ret=%d)",
          sdo.index, static_cast<unsigned>(sdo.sub_index),
          static_cast<unsigned>(slave_pos), ret);
      } else {
        RCLCPP_INFO(
          rclcpp::get_logger("EthercatDriver"),
          "Queued SDO 0x%04X:%u (%s, value=%d) for module at position %u",
          sdo.index, static_cast<unsigned>(sdo.sub_index),
          sdo.data_type.c_str(), sdo.data,
          static_cast<unsigned>(slave_pos));
      }
    }
  }
