...
{
  if (master == NULL) {
    EcMaster::printWarning("Failed to create domain: master is null");
    return;
  }
...
{
  if (master_ == NULL) {
    printWarning("Add slave failed: master not obtained (master is null).");
    return;
  }
...
{
  if (master_ == NULL) {
    printWarning("SDO download failed: master not obtained (master is null).");
...
{
  if (master_ == NULL) {
    printWarning("Activate. Failed to activate: master not obtained (master is null).");
    return false;
  }
  // register domain
...
{
  if (master_ == NULL) {
    printWarning("Update failed: master not obtained (master is null).");
    return;
  }
...
{
  if (master_ == NULL) {
    printWarning("Read failed: master not obtained (master is null).");
    return;
  }
  // receive process data
  ecrt_master_receive(master_);
...
{
  if (master_ == NULL) {
    printWarning("Write failed: master not obtained (master is null).");
    return;
  }
  DomainInfo * domain_info = domain_info_.at(domain);
  if (domain_info == NULL) {
