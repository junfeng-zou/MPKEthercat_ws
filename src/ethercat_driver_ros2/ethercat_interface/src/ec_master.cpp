...
int EcMaster::queueSlaveSdo(
  uint16_t slave_position, SdoConfigEntry sdo_config)
{
  // Locate the slave_config that addSlave() created for this position.
  ec_slave_config_t * cfg = NULL;
  for (auto & si : slave_info_) {
    if (si.position == slave_position) {
      cfg = si.config;
      break;
    }
  }
  if (cfg == NULL) {
    printWarning(
      "queueSlaveSdo: no slave config for position " +
      std::to_string(slave_position) +
      ". Did you call addSlave() first?");
    return -1;
  }

  uint8_t buffer[8];
  sdo_config.buffer_write(buffer);
  // ecrt_slave_config_sdo() returns 0 on success, negative on error.
  // The actual SDO transfer happens later, while the master walks the
  // bus from PREOP -> SAFEOP at activation time.
