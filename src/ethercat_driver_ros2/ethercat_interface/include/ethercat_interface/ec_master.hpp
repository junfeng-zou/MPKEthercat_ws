...
public:
  ...
  /** \brief configure slave using SDO (live download, requires bus to be in OP)
    *
    * NOTE: When called BEFORE master activation while slaves are still in
    * PREOP, this typically fails with "Invalid argument" (EINVAL) because the
    * underlying ecrt_master_sdo_download() requires an addressable slave that
    * is past PREOP. Prefer queueSlaveSdo() for startup configuration.
    */
  int configSlaveSdo(uint16_t slave_position, SdoConfigEntry sdo_config, uint32_t * abort_code);

  /** \brief queue an SDO to be written by the master during PREOP -> SAFEOP
    *
    * Wraps IgH's ecrt_slave_config_sdo() which queues the SDO download into
    * the slave_config object created by addSlave(). The master executes the
    * queued SDO downloads automatically as part of state-machine progression
    * to OP. This is the only reliable way to set timing-critical objects
    * (e.g. 0x60c2 interpolation_time_period) at startup.
    *
    * Returns 0 on success, negative on failure.
    *
    * IMPORTANT: must be called AFTER addSlave() (slave_config exists) and
    * BEFORE activate() (queue is committed at activation).
    */
  int queueSlaveSdo(uint16_t slave_position, SdoConfigEntry sdo_config);
