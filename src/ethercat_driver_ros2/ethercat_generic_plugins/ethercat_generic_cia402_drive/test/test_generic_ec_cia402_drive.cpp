...
TEST_F(EcCiA402DriveTest, EcWriteProfilePositionCommand)
{
  std::unordered_map<std::string, std::string> slave_paramters;
  std::vector<double> command_interface = {777777};
  ...
  plugin_->setup_interface_mapping();
  plugin_->is_operational_ = true;
  plugin_->mode_of_operation_display_ = 1;
  plugin_->last_position_ = 123456;

  uint8_t domain_address[4];
  EC_WRITE_S32(domain_address, 0);
  plugin_->processData(0, domain_address);

  ASSERT_EQ(EC_READ_S32(domain_address), 777777);
}
